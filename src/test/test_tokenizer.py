from src.main.tokens import Tokenizer, Token, QueryTerm
from src.main import LookaheadQueue
...
def test_tokenizer_prologue() -> None:
    ...
    tokens: LookaheadQueue[Token] = tokenizer.tokenize(query_str)
    
    assert tokens.get(block=False).term == QueryTerm.PREFIX
    prefix_name: Token = tokens.get(block=False)
    assert prefix_name.term == QueryTerm.PREFIX_NAME
