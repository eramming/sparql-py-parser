from src import Prologue, DatasetClause, SelectClause, GraphGraphPattern, \
    ...
    PathOp, PathMod, NegationExpr, GroupConcatFunction, SubSelect
...
def test_ggp_to_str() -> None:
    ...
    sel_clause.is_select_all()
    sub_sel.set_select_clause(SelectClause())
    sub_sel.set_where_clause(GroupGraphPatternSub(patterns=[gr_gr_1]))
...
def test_simple_expr_to_str() -> None:
    ...
    func_2arg: Function = Function("SUBSTR", [TerminalExpr(word), TerminalExpr(indx)])
...
def test_multi_expr_to_str() -> None:
    ...
    e_sub1 = MultiExprExpr(IdentityFunction(e_sub2), TerminalExpr(num3), ExprOp("!="))
    expr = IdentityFunction(MultiExprExpr(IdentityFunction(e_sub1), TerminalExpr("true"), ExprOp("||")))
